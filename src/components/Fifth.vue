<template>
      ...
      <div class="question"><p>QUESTION 5:</p> <p>{{ fifth.question }}</p></div>
      <div class="option" @click="checkAnswer($event)">A:  {{fifth.A }}</div>
      <div class="option" @click="checkAnswer($event)">B:  {{fifth.B }}</div>
      <div class="option" @click="checkAnswer($event)">C:  {{fifth.C }}</div>
      <div class="option" @click="checkAnswer($event)">D:  {{fifth.D }}</div>
...
<script>
      export default {
            ...
            methods: {
                  ...
                  checkAnswer($event){
                        if(this.click === 0){ 
                              if($event.target.textContent.startsWith(this.fifth.answer)){
