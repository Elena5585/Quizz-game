<template>
      ...
      <div class="question"><p>QUESTION 4:</p> <p>{{ fourth.question }}</p></div>
      <div class="option" @click="checkAnswer($event)">A:  {{fourth.A }}</div>
      <div class="option" @click="checkAnswer($event)">B:  {{fourth.B }}</div>
      <div class="option" @click="checkAnswer($event)">C:  {{fourth.C }}</div>
      <div class="option" @click="checkAnswer($event)">D:  {{fourth.D }}</div>
...
<script>
      export default {
            ...
            methods: {
                  ...
                  checkAnswer($event){
                        if(this.click === 0){ 
                              if($event.target.textContent.startsWith(this.fourth.answer)){
