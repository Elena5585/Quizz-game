<template>
      ...
      <div class="question"><p>QUESTION 2:</p> <p>{{ second.question }}</p></div>
      <div class="option a" @click="checkAnswer($event)">A:  {{second.A }}</div>
      <div class="option b" @click="checkAnswer($event)">B:  {{second.B }}</div>
      <div class="option c" @click="checkAnswer($event)">C:  {{second.C }}</div>
      <div class="option d" @click="checkAnswer($event)">D:  {{second.D }}</div>
...
<script>
      export default {
            ...
            methods: {
                  ...
                  checkAnswer($event){
                        if(this.click === 0){ 
                              if($event.target.textContent.startsWith(this.second.answer)){
