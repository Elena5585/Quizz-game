<template>
      ...
      <div class="question"><p>QUESTION 3:</p> <p>{{ third.question }}</p></div>
      <div class="option" @click="checkAnswer($event)">A:  {{third.A }}</div>
      <div class="option" @click="checkAnswer($event)">B:  {{third.B }}</div>
      <div class="option" @click="checkAnswer($event)">C:  {{third.C }}</div>
      <div class="option" @click="checkAnswer($event)">D:  {{third.D }}</div>
...
<script>
      export default {
            ...
            methods: {
                  ...
                  checkAnswer($event){
                        if(this.click === 0){ 
                              if($event.target.textContent.startsWith(this.third.answer)){
